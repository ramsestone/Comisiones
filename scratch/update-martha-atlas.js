...
async function run() {
  const uri = 'mongodb+srv://ti_db_user:CAQRpTS4CRoUy3qM@commission-management.e8ueb0g.mongodb.net/?appName=Commission-Management';
  ...
  try {
    ...
    const comision = await comisionesColl.findOne({
      client_name: { $regex: /MARTHA LAURA/i }
    });

    if (!comision) {
      console.log('Comisión no encontrada para MARTHA LAURA en ATLAS');
      return;
    }

    console.log('Comisión encontrada en ATLAS:', comision._id, comision.client_name);
